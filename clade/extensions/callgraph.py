        ...
        for line in self.extensions["Info"].iter_calls():
            ...
            for files in (
                    ...
                    (f for f in possible_files if self._files_are_linked(f, context_file) and
                        any(self._t_unit_is_common(cf, context_file) for cf in self.funcs[func][f].get("declarations", []))) if call_type == "global" else tuple(),
                    (f for f in possible_files if any(self._t_unit_is_common(cf, context_file) for cf in self.funcs[func][f].get("declarations", [])))
